...
- WebDriver server (`webdriver` subcommand, compiled only with the off-by-default `webdriver` cargo feature) routes every navigation through the same engine and therefore inherits the SSRF protection, URL scheme whitelist, and navigation-delegate checks; it binds `127.0.0.1:4444` by default (explicit `--host` required to expose) and the underlying `webdriver` crate only accepts requests whose `Host` header is `localhost` or an IP literal. It has **no authentication** — anyone who can reach the port can drive a real browser engine and execute arbitrary JavaScript, so expose it only behind a trusted boundary. Reaching loopback/private targets (e.g. a local test server) requires the explicit `--allow-private-addresses` flag, which disables SSRF protection process-wide.
